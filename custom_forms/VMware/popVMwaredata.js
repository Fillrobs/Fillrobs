function popVMwaredata(EnvID) {
  ...
  survey.onCurrentPageChanged.add(function (sender, options) {
    if (options.newCurrentPage.name === "VMware Environment Groups") {
      ...
      if (EnvID !== null) {
        ...
        // end of if current page is vmware_environment groups
      }
  ...
  try {
    CI_VMware_environment_id = survey.getValue("CI_VMware_environment_id");
    //console.log("CI_VMware_environment_id(a)=" + CI_VMware_environment_id);
  } catch {
    CI_VMware_environment_id = EnvID;
  }
  ...
  //console.log("CI_VMware_environment_id(b)=" + CI_VMware_environment_id);
  getval_url =
    "http://localhost:8001/api/v3/cmp/inboundWebHooks/IWH-p9fkr0q9/run/?CI_VMware_environment_id=" +
    CI_VMware_environment_id +
    "&token=40D6y7K3vxxPyT0dckg-KRotziqIeI1Xds46o4W18RU";
  $.ajax({
    url: getval_url,
    type: "GET",
    success: function (data) {
      console.log(data);
      survey.setValue("CI_vmwareEnvId", data[0]["vmwareEnvId"]);
      survey.setValue("CI_vmwareEnvName", data[0]["vmwareEnvName"]);
      survey.setValue("CI_rhName", data[0]["rhName"]);
      // vmware Parameters
      var fieldArray = [
        "vmwareDatastoresData",
        "vmwareDiskTypeData",
        "envGroupData",
        "envNetworkData",
        "envOSBuildData",
      ...
      group_data = data[0][fieldArray[2]];
      ...
      if (group_data_len > 0) {
        ...
        for (g = 0; g < group_data_len; g++) {
          var dataObj = {
            ...
            EnvID: data[0]["vmwareEnvId"],
          };
          dataArr.push(dataObj);
          sessionStorage.setItem("EnvID", data[0]["vmwareEnvId"]);
          console.log("set EnvID=" + data[0]["vmwareEnvId"]);
        }
