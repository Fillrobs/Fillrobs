function popAzuredata(EnvID) {
  ...
  survey.onCurrentPageChanged.add(function (sender, options) {
    if (options.newCurrentPage.name === "Azure Environment Groups") {
      ...
      if (EnvID !== null) {
        ...
        // end of if current page is Azure_environment groups
      }
  ...
  try {
    CI_Azure_environment_id = survey.getValue("CI_Azure_environment_id");
    //console.log("CI_Azure_environment_id(a)=" + CI_Azure_environment_id);
  } catch {
    CI_Azure_environment_id = EnvID;
  }
  ...
  //console.log("CI_Azure_environment_id(b)=" + CI_Azure_environment_id);
  getval_url =
    "http://localhost:8001/api/v3/cmp/inboundWebHooks/IWH-w6puf264/run/?CI_Azure_environment_id=" +
    CI_Azure_environment_id +
    "&token=YyiZ5zbX1ZdlBgde2fU4wxHgHf8Vakr6m9eV_j4tuB8";
  $.ajax({
    url: getval_url,
    type: "GET",
    success: function (data) {
      console.log(data);
      survey.setValue("CI_azureEnvId", data[0]["AzureEnvId"]);
      survey.setValue("CI_azureEnvName", data[0]["azureEnvName"]);
      survey.setValue("CI_azureServiceAccount", data[0]["azureServiceAccount"]);
      survey.setValue("CI_azureClientID", data[0]["azureClientID"]);
      survey.setValue("CI_rhName", data[0]["rhName"]);
      survey.setValue("CI_azureTenantID", data[0]["azureTenantID"]);
      // Azure Parameters
      var fieldArray = [
      ...
      group_data = data[0][fieldArray[11]];
      ...
      if (group_data_len > 0) {
        ...
        for (g = 0; g < group_data_len; g++) {
          var dataObj = {
            ...
            EnvID: data[0]["AzureEnvId"],
          };
          dataArr.push(dataObj);
          sessionStorage.setItem("EnvID", data[0]["AzureEnvId"]);
          console.log("set EnvID=" + data[0]["AzureEnvId"]);
        }
