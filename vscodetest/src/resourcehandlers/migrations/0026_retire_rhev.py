...
def get_id_to_remove(apps, schema_editor):
    ...
    if ResourceTechnology.objects.filter(name="Red Hat Enterprise Virtualization").exists():
        return ResourceTechnology.objects.get(name="Red Hat Enterprise Virtualization").id
    else:
        return -1


def remove_rhev_networks(apps, schema_editor):
    ResourceNetwork = apps.get_model('resourcehandlers', 'ResourceNetwork')
    if ResourceNetwork.objects.filter(real_type__model="rhevnetwork").exists():
        ResourceNetwork.objects.filter(real_type__model="rhevnetwork").delete()


def remove_rhev_from_technology(apps, schema_editor):
    ResourceTechnology = apps.get_model('resourcehandlers', 'ResourceTechnology')
    if ResourceTechnology.objects.filter(name="Red Hat Enterprise Virtualization").exists():
        ResourceTechnology.objects.filter(name="Red Hat Enterprise Virtualization").delete()


def remove_rhev_resource_handlers(apps, schema_editor):
...
class Migration(migrations.Migration):
    ...
    if 'rhev_rhevresourcehandler_networks' in all_tables:
        operations.append(migrations.RunSQL(
            sql=['DELETE FROM rhev_rhevresourcehandler_networks;', ],
            reverse_sql=[],)
        )
    operations.append(migrations.RunSQL(
        sql=['DROP TABLE IF EXISTS rhev_rhevresourcehandler_networks CASCADE;'],
        reverse_sql=[""" DROP TABLE IF EXISTS rhev_rhevresourcehandler_networks CASCADE;
            CREATE TABLE `rhev_rhevresourcehandler_networks` (
            `id` int(11) NOT NULL AUTO_INCREMENT,
            `rhevresourcehandler_id` int(11) NOT NULL,
            `rhevnetwork_id` int(11) NOT NULL,
            PRIMARY KEY (`id`));
        """],)
    )

    if 'rhev_rhevnetwork' in all_tables:
        operations.append(migrations.RunSQL(
            sql=['DELETE FROM rhev_rhevnetwork;', ],
            reverse_sql=[], )
        )
    operations.append(migrations.RunSQL(
        sql=['DROP TABLE IF EXISTS rhev_rhevnetwork CASCADE;'],
        reverse_sql=[""" DROP TABLE IF EXISTS rhev_rhevnetwork CASCADE;
            CREATE TABLE `rhev_rhevnetwork` (
    ...
    operations.append(migrations.RunSQL(
        sql=["DROP TABLE IF EXISTS rhev_rhevresourcehandler_os_build_attributes CASCADE;"],
        reverse_sql=[],
    ))

    if 'rhev_rhevosbuildattribute' in all_tables:
        operations.append(migrations.RunSQL(
            sql=['DELETE FROM rhev_rhevosbuildattribute;', ],
            reverse_sql=[], )
        )
    operations.append(migrations.RunSQL(
        sql=['DROP TABLE IF EXISTS rhev_rhevosbuildattribute CASCADE;'],
        reverse_sql=["""  DROP TABLE IF EXISTS rhev_rhevosbuildattribute CASCADE;
            CREATE TABLE `rhev_rhevosbuildattribute` (
    ...
    if 'rhev_rhevresourcehandler' in all_tables:
        operations.append(migrations.RunSQL(
            sql=['DELETE FROM rhev_rhevresourcehandler;', ],
            reverse_sql=[],)
        )
    operations.append(migrations.RunSQL(
        sql=['DROP TABLE IF EXISTS rhev_rhevresourcehandler CASCADE;'],
        reverse_sql=["""  DROP TABLE IF EXISTS rhev_rhevresourcehandler CASCADE;
            CREATE TABLE `rhev_rhevresourcehandler` (
    ...
    operations.append(migrations.RunPython(remove_rhev_networks, noop))
    operations.append(migrations.RunPython(remove_rhev_resource_handlers, noop))
    operations.append(migrations.RunPython(remove_rhev_from_technology, noop))
