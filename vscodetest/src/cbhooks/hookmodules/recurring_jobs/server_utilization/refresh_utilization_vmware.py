...
from resourcehandlers.utilization import VMwareSamplingPeriods
...
def split_job_into_one_per_resource_handler(parent_job, action):
    rhs = VsphereResourceHandler.objects.all()
    rh_count = len(rhs)
    if rh_count == 0:
        return "SUCCESS", "No vCenter resource handlers found, nothing to do.", ""
    ...
    set_progress(
        "Refresh Utilization for Servers in {} VMware resource handlers".format(
            rh_count
        ),
...
def run(job, logger=None, action=None, **kwargs):
    ...
    For all VMWare resource handlers, get utilization stats for each active server and save them in
    ...
    if not resource_handler_id:
        # spin off jobs for each VMware resource_handler
        return split_job_into_one_per_resource_handler(job, action)

    updated_count = 0
    skipped_count = 0
    metrics = dict(
        last_year=VMwareSamplingPeriods.last_year,
        last_month=VMwareSamplingPeriods.last_month,
        last_day=VMwareSamplingPeriods.last_day,
        last_hour=VMwareSamplingPeriods.last_hour,
    )
    total_tasks = 1
    ...
    for server in servers:
        server_util = {}
        server_util["last_year"] = utilization["last_year"][server]
...
def get_utilization_for_servers(rh, servers, period):
    ...
    Args
        rh: VMware resource handler instance
    ...
    # we use enumerate here because vSphere sends each server's data back in
...
if __name__ == "__main__":
    ...
    For testing, call this directly like so:
        python refresh_utilization_vmware.py
    """
    run(None)
