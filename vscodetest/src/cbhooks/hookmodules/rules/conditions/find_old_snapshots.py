...
def format_snapshot_desc(snapshot):
    ...
    """
    create_date = "{}/{}/{}".format(
        snapshot.createTime.year, snapshot.createTime.month, snapshot.createTime.day
    )
...
def get_snapshots_for_handler(rh, age_threshold):
    ...
    servers = rh.server_set.exclude(status__in=["HISTORICAL", "DECOM", "PROVFAILED"])
    all_shots = []
    for server in servers:
        ...
        if shots:
            ...
            set_progress(
                "  VM '{}' has {}: {}".format(server, len(shots), ", ".join(shot_descs))
            )

            shots = [(server.id, shot.id) for shot in shots]
            all_shots.extend(shots)

    set_progress("Found {} matching snapshots for {}".format(len(all_shots), rh))
    return all_shots
...
def check(job, logger, **kwargs):
    ...
    snaps_to_delete = {"snapshots": all_shots} if all_shots and not DRY_RUN else {}
