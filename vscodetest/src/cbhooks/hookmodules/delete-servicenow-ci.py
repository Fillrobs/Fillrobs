...
"""
Sample hook to delete a CI from ServiceNow.
This hook has been deprecated. Use cbhooks/hookmodules/delete_service_now_ci.py instead.
"""
...
def run(job, logger=None):
    ...
    for server in job.server_set.all():
        ...
        if not sysid:
            job.set_progress(
                "Unable to locate {} in ServiceNow".format(server.hostname)
            )
            return "", "", ""
