#!/usr/bin/env python
# CloudBolt Plugin version of this pyvmomi community sample:
...
def print_http_nfc_lease_info(info):
    ...
    """
    debug(
        "Lease timeout: {0.leaseTimeout}\nDisk Capacity KB: {0.totalDiskCapacityInKB}".format(
            info
        )
    )
    device_number = 1
...
def convert(args):
    ...
    debug("Working dir: {} ".format(args.workdir))
    if not os.path.isdir(args.workdir):
        debug("Creating working directory {}".format(args.workdir))
        os.mkdir(args.workdir)
    # actual target directory for VM
    target_directory = os.path.join(args.workdir, vm_obj.config.instanceUuid)
    debug("Target dir: {}".format(target_directory))
    if not os.path.isdir(target_directory):
        debug("Creating target dir {}".format(target_directory))
    ...
    try:
            ...
            if http_nfc_lease.state == vim.HttpNfcLease.State.ready:
                ...
                for deviceUrl in http_nfc_lease.info.deviceUrl:
                    ...
                    temp_target_disk = os.path.join(
                        target_directory, deviceUrl.targetId
                    )
                    debug(
                        "Downloading {} to {}".format(deviceUrl.url, temp_target_disk)
                    )
                    ...
                    debug("Creating OVF file for {}".format(temp_target_disk))
                ...
                debug("HTTP NFC Lease Initializing.")
            elif http_nfc_lease.state == vim.HttpNfcLease.State.error:
                debug("HTTP NFC Lease error: {}".format(http_nfc_lease.state.error))
                sys.exit(1)
            sleep(2)
        debug("Getting OVF Manager")
        ovf_manager = si.content.ovfManager
        debug("Creating OVF Descriptor")
        ...
        if vm_descriptor_result.error:
            ...
            debug("Writing OVF Descriptor {}".format(target_ovf_descriptor_path))
...
def run(job, logger=None, **kwargs):
    cb_logger = logger  # noqa: F841
    cb_job = job  # noqa: F841
    for server in job.server_set.all():
        # Get vCenter connection for host,user,pass,port
        rh = server.resource_handler.cast()
        si = pyvmomi.get_connection(rh.ip, rh.port, rh.serviceaccount, rh.servicepasswd)
        # Get UUID from CB Server object
        uuid = server.resource_handler_svr_id
        # Persist a workdir
        workdir = "/tmp/ova/"
        # Specify name from ...?
        name = "{{ova_file_name}}" or server.hostname
        # Build args
        args = ConversionArgs(uuid=uuid, workdir=workdir, name=name, si=si)
