...
// Enables POST/PUT/PATCH requests via the browsable API
// from https://github.com/tomchristie/ajax-form
...
function doAjaxSubmit(e) {
    ...
    var ret = $.ajax({
        url: url,
        type: method,  // "type" for jQuery < 1.9, "method" for 1.9 and above
        data: data,
 ...
 * jQuery Formset 1.2
 *
 * Forked from https://github.com/elo80ka/django-dynamic-formset
 * and modified by CloudBolt.
 *
 *    ---
 *    Warning: this plugin was customized for CB
 *    and is no longer a general purpose solution
 *    ---
 *
...
;(function($) {
    $.fn.formset = function(opts)
    {
        var options = $.extend({}, $.fn.formset.defaults, opts),
            ...
            deleteForm = function(event) {
                ...
                if (del.length) {
                    ...
                    del.val('on');
                    if (!options.removedRowsClass) {
                        row.hide();
                    } else {
                        row.addClass(options.removedRowsClass);

                        $('a.unremove').off('click');
                        $(this).replaceWith(
                            '<span class="unremoveWrap">' +
                            '  <a class="unremove">' + gettext("Undo") + '</a>' +
                            '</span>');
                        $('a.unremove').on('click', unremoveRow);
                    }
                } else {
                    row.remove();
            ...
            unremoveRow = function(event) {
                event.preventDefault();
                var row = $(this).closest('.' + options.removedRowsClass);
                var del = row.find('input:hidden[id $= "-DELETE"]');
                del.val('');
                row.removeClass(options.removedRowsClass);
                $(this).closest('.unremoveWrap').remove();
                insertDeleteLink(row);
                if (options.unremoved) options.unremoved(row);
            },

            deleteAllForms = function(event) {
                event.preventDefault();
                var row = $(this).parents('.formset-controls');
                var rows = row.siblings();
                rows.find('.delete-form').trigger('click');
            },

            insertDeleteLink = function(row) {
                var $deleteLink = $('<a class="' + options.deleteClass + '">' +
                                    options.deleteLabel + '</a>');
                if (row.is('TR')) {
                    // If the forms are laid out in table rows, insert
                    // the remove button into the last table cell:
                    row.children(':last').append($deleteLink);
                    parent_sel = 'tr';
        ...
        if ($$.length) {
            ...
            var $controls;
            var $addLink = $(
                '<a class="'+ options.addClass + '">' + options.addLabel + '</a>'
            ).on('click', addForm);

            // Insert 'Add' and optional 'Delete All' links as appropriate
            // for the context
            var containerTag = $$.prop('tagName');
            if (containerTag == 'TR') {
                var numCols = $$.eq(0).children().length;
                var controls = '<tr class="formset-controls">';
                // Use for loop instead of repeat() for IE11 compatibility
                for (i=0; i < numCols-1; i++) {
                    controls += "<td></td>";
                }
                controls += '<td class="shrink text-center"></td>';
                controls += '</tr>';
                $controls = $(controls);
                $controls.find('td:eq(0)').append($addLink);

                if (options.showDeleteAllLink) {
                    var $deleteAllLink = $(
                        '<a class="' + options.deleteAllClass + '">' + options.deleteAllLabel + '</a>'
                    ).on('click', deleteAllForms);
                    var lastIndex = numCols-1;
                    $controls.find('td:eq('+lastIndex+')').append($deleteAllLink);
                }

                $$.parent().append($controls);
            } else if (containerTag == 'LI') {
                console.log('c2 jquery.formset does not support lists.');
            } else {
                // Otherwise, insert it immediately after the last form:
                $controls = $('<div class="formset-controls"></div>');
                $controls.append($addLink);
                if (options.showDeleteAllLink) {
                    $addLink.after($deleteAllLink);
                }
                $$.filter(':last').after($controls);
            }
            $controls.addClass(options.formCssClass + '-add');
        }
    ...
    $.fn.formset.defaults = {
        ...
        // *Label option sets the content of the anchor tag
        // *Class sets the CSS class or classes
        addLabel: '<span class="icon-add"></span> ' + gettext("Add Another"),
        addClass: 'btn btn-link add-form',

        deleteLabel: '<span class="icon-delete"></span>',
        deleteClass: 'btn btn-link delete-form',

        showDeleteAllLink: true,
        deleteAllLabel: '<span class="icon-delete"></span> ' + gettext("Remove All"),
        deleteAllClass: 'btn btn-link delete-forms',

        formCssClass: 'dynamic-form',    // CSS class applied to each form in a formset
        extraClasses: [],                // Additional CSS classes, which will be applied to each form in turn
        added: null,                     // Function called each time a new form is added. Row is passed.
        removed: null,                   // Function called each time a form is deleted. Row is passed.
        unremoved: null,                 // Callback for when a pending removal is canceled. Row is passed.
        hideRemovedRows: true,           // If false, make sure to indicate state via removed callback
        removedRowsClass: 'toBeRemoved'  // If not empty, mark the row instead of hiding; also change the link to cancel
    };
})(jQuery)
