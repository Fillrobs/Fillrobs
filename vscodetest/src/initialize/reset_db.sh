...
# errors in drop are ignorable (it may just be that the DB didn't exist)
echo "DROP DATABASE $dbname" | mysql -u "$dbuser" -p"$dbpass" -h"$dbhost" -P"$dbport" 2>&1
database_delete_return=$?
echo "* database_delete_return: $database_delete_return"
...
database_create_return=$?
echo "* database_create_return: $database_create_return"
test $database_create_return -gt 0 && exit $database_create_return
